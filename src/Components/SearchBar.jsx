...
export default function SearchBar(){
  ...
  useEffect(()=>{
    for(let page=0;page<=20;page++)
    {
      const options={
        method:'GET',
        url:`https://api.themoviedb.org/3/discover/movie?api_key=97fadb736a602ca05a4e0b04494ea6c3&page=${page+1}`,
        headers:{
  ...
  const showSuggestions=()=>{
    ...
    const filteredSuggestions=searchMovies.filter(item=>item.original_title.toLowerCase().includes(input))
    if (filteredSuggestions.length > 0) {
      filteredSuggestions.forEach(suggestion => {
        const listItem = document.createElement("li");
        listItem.className =
          "px-4 py-2 hover:bg-gray-200 cursor-pointer";
        listItem.textContent = suggestion.original_title;
